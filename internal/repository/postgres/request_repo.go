...
func (r *requestRepo) Get() ([]repository.Request, error) {
	...
	`)
	if err != nil {
		r.Logger.Error("failed to query requests", slog.Any("err", err))
		return nil, err
	}
	defer rows.Close()
...
func (r *requestRepo) Create(request repository.Request) (int64, error) {
	...
	r.Logger.Debug("request created", slog.Int64("id", id), slog.String("user", request.Name))
	return id, nil
}
...
func (r *requestRepo) Update(request repository.Request) error {
	_, err := r.DB.Exec(`
		UPDATE requests
		SET name = $1, phone = $2, email = $3, contact_type = $4, message = $5, updated_at = $6
		WHERE id = $7
	`,
		request.Name,
		request.Phone,
