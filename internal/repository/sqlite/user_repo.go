...
func (r *userRepo) Create(user repository.User) (int64, error) {
	result, err := r.DB.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?)
	`, user.Username, user.Password, user.Email, 1, time.Now(), time.Now())
	if err != nil {
...
func (r *userRepo) Update(user repository.User) error {
	...
	r.Logger.Debug("user updated", slog.Int64("id", user.ID), slog.String("username", user.Username), slog.Any("err", err))
	return nil
}
