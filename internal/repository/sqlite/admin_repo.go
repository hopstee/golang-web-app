...
func (r *adminRepo) Create(admin repository.Admin) (int64, error) {
	result, err := r.DB.Exec(`
		...
		VALUES (?, ?, ?, ?, ?)
	`, admin.Username, admin.Password, admin.Role, time.Now(), time.Now())
	if err != nil {
...
func (r *adminRepo) Update(admin repository.Admin) error {
	...
	r.Logger.Debug("admin updated", slog.Int64("id", admin.ID), slog.String("username", admin.Username), slog.Any("err", err))
	return nil
}
